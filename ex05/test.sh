...
time_limit["echo"]=1s
time_limit["line"]=1s
time_limit["popular"]=1s
time_limit["contact"]=1s
time_limit["social"]=1s
...
do
	...
	then
		...
		then
			...
			if [ -e "$out" ] 
			then
				if [ "$PROG" == "line" ] # TODO replace with name of the program as appropriate
				then
...
then
	if [ "$PROG" == "max" ] 
	then
		check_has_function "$PROG" "find_max" "long (const long \*, long, long)"
		if [ "$g_has_function" -eq 0 ]
		then
			echo "$PROG: the expected function find_max cannot be found"
		else
