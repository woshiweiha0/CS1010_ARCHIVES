...
do
	...
	then
		make_temp
		timeout 1s ./"$PROG" < "inputs/$PROG.$i.in" | head -c 50MB > "$out"
		status="${PIPESTATUS[0]}"
		if [ "$status" -ne "0" ]
		then
			if [ "$status" -eq 124 ]; then
				echo "$PROG: took longer than 1s for test case $i"
			else
...
then
	if [ "$PROG" == "odd" ] 
	then
		check_has_function odd "find_next_odd" "long (long)"
		if [ "$g_has_function" -eq 0 ]
		then
			echo "$PROG: failed. The expected function find_next_odd cannot be found"
		else
			echo "$PROG: passed"
		fi
	elif [ "$PROG" == "sum" ] 
	then
		check_has_function sum "compute_sum_if_positive" "long (long, long)"
		if [ "$g_has_function" -eq 0 ]
		then
			echo "$PROG: failed. The expected function compute_sum_if_positive cannot be found"
		else
			echo "$PROG: passed"
		fi
	elif [ "$PROG" == "triangle" ]
	then
		check_has_function triangle "is_possible_triangle" "bool (long, long, long)"
		if [ "$g_has_function" -eq 0 ]
		then
			check_has_function triangle "is_possible_triangle" "_Bool (long, long, long)"
			if [ "$g_has_function" -eq 0 ]
			then
				echo "$PROG: failed. The expected function is_possible_triangle cannot be found"
			else
				echo "$PROG: passed"
			fi
		else
			check_has_function triangle "compute_area" "double (long, long, long)"
			if [ "$g_has_function" -eq 0 ]
			then
				echo "$PROG: failed. The expected function compute_area cannot be found"
			else
				echo "$PROG: passed"
			fi
		fi
